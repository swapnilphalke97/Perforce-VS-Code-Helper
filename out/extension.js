...
function runP4Command(command, filePath) {
    ...
    vscode.window.withProgress({
        ...
        return new Promise((resolve) => {
            (0, child_process_1.exec)(p4Cmd, { cwd }, (error, stdout, stderr) => {
                if (error) {
...
function activate(context) {
    let editDisposable = vscode.commands.registerCommand('p4-vscode-helper.edit', (uri) => {
    ...
    let addDisposable = vscode.commands.registerCommand('p4-vscode-helper.add', (uri) => {
        const filePath = getActiveFilePath(uri);
        if (filePath) {
    ...
    let revertDisposable = vscode.commands.registerCommand('p4-vscode-helper.revert', (uri) => {
        const filePath = getActiveFilePath(uri);
        if (filePath) {
